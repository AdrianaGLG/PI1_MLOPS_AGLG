...
async def startup():
    global df_parte, df_Total, proyecto1_crew, proyecto1_movies
...
def extract_data_from_zip(): # para extraer los archivos comprimidos
    return df_parte, df_Total, proyecto1_crew, proyecto1_movies
...
# Luego de la etapa de EDA se obtuvo un dataset que agrupa ambos csv previamente tratados
# En este caso podemos definir la variable:
# todo = pd.read_csv('df_Total.csv', encoding='latin-1')
...
def cantidad_filmaciones_mes(Mes: str) -> int:
    ...
    _, _, _, proyecto1_movies = extract_data_from_zip()
...
def cantidad_filmaciones_dia(Dia: str) -> str:
    _, df_Total, _, _ = extract_data_from_zip()
    ...
    if dia_ingles:
        ...
        peliculas_dia = df_Total[df_Total['release_date'].dt.day_name() == dia_ingles]
...
def score_titulo(titulo_de_la_filmacion: str) -> str:
    _, df_Total, _, _ = extract_data_from_zip()
    pelicula = df_Total[df_Total['title'] == titulo_de_la_filmacion]
    if len(pelicula) > 0:
...
def get_actor(nombre_actor: str):
    ...
    actor_credits = df_Total[df_Total['Name_actors'].str.contains(nombre_actor, case=False, na=False)]
    ...
    actor_movies = df_Total[df_Total['id'].isin(actor_credits['id'])]
...
def get_director(nombre_director: str):
    ...
    # Crear una copia del DataFrame "todo" y convertir la columna "director" a minúsculas
    df_director = df_Total.copy()
...
def calcular_similitud():
    ...
    overview_features = tfidf.fit_transform(df_Total["overview"].astype(str))
